...
sub = read.table("figure2_prep_feb2017.txt", sep = "\t", header = T, stringsAsFactors = F)
...
mod = read.table("editsFeb2017.csv", sep = ",", header = T, stringsAsFactors = F)
...
p = ggplot(data = summary_table, aes(x = Subsystem, y = `total reactions`)) +
  ...
  theme(axis.title.x = element_text(size = 14),
        ...
        plot.margin = unit(c(10,10,10,10),"mm"),
        legend.title=element_blank(),
        legend.position = c(.82, 0.8),
        legend.text=element_text(size=14))  
p = p + guides(fill= guide_legend(reverse=T)) 

setwd(paste(base_directory,'/figures',sep=""))
tiff(filename = "Fig2_curation_summary.tiff", width = 700, height = 320)
plot(p)
dev.off()
...
file_v = read.csv("gene_states_v_feb2017.csv", sep = ',', header = T, stringsAsFactors = F)
file = merge(file_c,file_v, by.x = 'genes', by.y = 'genes')
grs = read.table("GeneRxnSubsystems_feb2017.txt", sep = "\t", header = T, stringsAsFactors = F)
...
model = read.table("figure2_prep_feb2017.txt", sep = "\t", header = T, stringsAsFactors = F)
...
plot = ggplot(file2, aes(x = Var2, y = Var1)) + 
  ...
  coord_flip()  
setwd(paste(base_directory,'/figures',sep=""))
tiff(filename = "SF4_enrichment.tiff", width = 715, height = 280)
plot(plot)
dev.off()
